...
from ctypes import pointer, byref, c_float, c_long, c_ulong, c_buffer
...
class Channel (FlagObject):
	...
	def remove_fx(self, fx):
		"""Remove an effect on a stream, MOD music, or recording channel."" from handle of set_fx when it was returned"""
		return bass_call(BASS_ChannelRemoveFX, self.handle, fx)

	def set_fx_setparam(self, handle, params):
		return bass_call(BASS_FXSetParameters, handle, pointer(params))

	def set_fx_getparam(self, handle, params):
		bass_call(BASS_FXGetParameters, handle, pointer(params))
