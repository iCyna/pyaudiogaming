...
class Output (object):
	...
	def set_device(self, device):
		if device == self._device or device is None:
			return
		if isinstance(device, str):
			device = self.find_device_by_name(device)
		self.free()

		self.init_device(device=device)
		self._device = device
		return bass_call(BASS_SetDevice, self.device)

	device = property(fget=get_device, fset=set_device)

	def get_volume (self):
		stream_volume = BASS_GetConfig(BASS_CONFIG_GVOL_STREAM)
		sample_volume = BASS_GetConfig(BASS_CONFIG_GVOL_SAMPLE)
		if stream_volume and sample_volume:
			stream_volume /= 100.0
			sample_volume /= 100.0
		return stream_volume, sample_volume
	...
	def set_volume (self, volume):
		...
		if volume is not None:
			...
			# Set volume for sample
			bass_call(BASS_SetConfig, BASS_CONFIG_GVOL_SAMPLE, int(round(volume*100, 2)))
	...
	def find_device_by_id(self, device_id):
		info = BASS_DEVICEINFO()
		if BASS_GetDeviceInfo(device_id, ctypes.byref(info)):
			retrieved = info.name
			if platform.system() == 'Windows':
				retrieved = retrieved.decode('mbcs')
			elif platform.system() == 'Darwin':
...
class ThreeDOutput(Output):
	...
	def set_eax_parameters(self, environment="city", volume=None, decay=200, damp=None):
		def convert_arg(arg):
			if arg is None:
				arg = -1
			return arg
