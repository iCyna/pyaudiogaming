...
class URLStream(BaseStream):
	...
	def __init__(self, url="", offset=0, flags=0, downloadproc=None, user=None, mono=False, three_d=False, autofree=False, decode=False, unicode=True):
		...
		self.url = url
		self.setup_flag_mapping()
		flags = flags | self.flags_for(mono=mono, three_d=three_d, autofree=autofree, decode=decode, unicode=unicode)
		offset = int(offset)
